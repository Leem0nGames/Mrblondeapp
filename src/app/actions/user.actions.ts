...
import type { CartItem, AuthState, AppSettingsRow } from '@/types';
...
export async function getOrderPageData(agreementId: string) {
    ...
    if (agreementError || !agreement) return { data: null, error: { message: "Convenio inválido." } };
    ...
    if (itemsError) return { data: null, error: { message: "Error al cargar productos." } };
